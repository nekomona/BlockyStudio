...
# Regex pattern
pattern = 'set_pin_assignment\s+\{\s*(.*?)\s*}\s*\{[a-zA-Z0-9=;\s]*LOCATION\s*=\s*(.*?)\s*;[a-zA-Z0-9=;\s]*}\s+(#+\s+(input|output))?'
# pattern = 'set_io\s+(--warn-no-port)?\s*(.*?)\s+(.*?)\s+(#+\s+(input|output))?'

# Open file
with open(os.path.join(path, 'pinout.adc')) as file:
    data = file.read()
    ...
    # pinout=sorted(pinout, key=lambda pinout: pinout[0],reverse=True)
